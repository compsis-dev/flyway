...
CREATE TABLE ${table} (
    ...
    ${versionColumn} VARCHAR(50) NULL,
    ${descriptionColumn} VARCHAR(200) NOT NULL,
    ${typeColumn} VARCHAR(20) NOT NULL,
    ${scriptColumn} VARCHAR(1000) NOT NULL,
    ${checksumColumn} INT NULL,
    ${installedByColumn} VARCHAR(100) NOT NULL,
    ${installedOnColumn} datetime DEFAULT getDate() NOT NULL,
    ${executionTimeColumn} INT NOT NULL,
    ${successColumn} decimal NOT NULL,
    PRIMARY KEY (${installedRankColumn})
)
lock datarows on 'default'
go

CREATE INDEX ${table}_s_idx ON ${table} (${successColumn})
go
